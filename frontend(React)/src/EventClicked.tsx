...
export default function EventClicked() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:3000/event/${resolvedId}`, {
      ...
      .then((res) => {
        if (!res.ok) {
          throw new Error(`Failed to load event ${resolvedId}`)
        }
        return res.json()
      })
  ...
  function primaryAction() {
    ...
    if (eventNeedsTicket(events)) {
      ...
      /*if (!user) {
        navigate(`/login?from=${encodeURIComponent(`/event/${eventIdValue}`)}`)
        return
      }*/
            ...
            <div className="relative overflow-hidden rounded-2xl border border-border/40 bg-muted/20 shadow-[0_0_0_1px_rgba(255,255,255,0.04)]">
              ...
              <div className="absolute right-3 top-3 flex gap-2">
                <Button
                  ...
                  size="icon"
                  variant="secondary"
                  className={saved ? 'h-10 w-10 rounded-full border-0 bg-black/50 text-white backdrop-blur-md hover:bg-black/65 text-pink-400' : 'h-10 w-10 rounded-full border-0 bg-black/50 text-white backdrop-blur-md hover:bg-black/65'}
                  aria-label={saved ? 'Remove from saved' : 'Save event'}
                  onClick={() => void toggleSave()}
                >
                  <Heart className={saved ? 'h-4 w-4 fill-current' : 'h-4 w-4'} strokeWidth={1.75} />
                </Button>
                <Button
                  type="button"
                  size="icon"
              ...
              <section>
                ...
                {aboutLong ? (
                  ...
                  >
                    ...
                    <ChevronDown className={aboutExpanded ? 'h-4 w-4 rotate-180 transition-transform' : 'h-4 w-4 transition-transform'} />
                  </button>
                ) : null}
