...
export default function PurchasedTicketScreen() {
  ...
  const reservationQrData = useMemo(() => {
    ...
    if (looksLikeReservationUuid(rid)) return `reservation:${rid}`
  ...
  async function handleDownloadPdf() {
    if (isReservation) {
      if (!reservationQrData) {
        Alert.alert('PDF unavailable', 'No reservation QR is available yet.')
        return
      }
      try {
        await downloadTicketPdf({
          reservationId: reservationDisplayUuid ?? oneQueryParam(params.reservationId) ?? '',
          eventName: params.eventName ?? 'Event',
          ticketTypeName: params.ticketTypeName ?? 'Table reservation',
          quantity: params.quantity ?? '1',
          total: params.total,
          isReservation: true,
          qrCode: reservationQrData,
          attendees: [],
          status: 'Pending',
        })
      } catch (e: unknown) {
        const msg = e instanceof Error ? e.message : 'Could not create the ticket PDF.'
        Alert.alert('PDF unavailable', msg)
      }
      return
    }

    if (loadingIds) {
      Alert.alert('Preparing ticket', 'Please wait for your ticket codes to load.')
      return
    }
    if (ticketQrPayloads.length === 0) {
      Alert.alert('PDF unavailable', 'Ticket codes are not ready yet. Check Bookings in a moment.')
      return
    }
    try {
      await downloadTicketPdf({
        ...
        quantity: params.quantity ?? String(ticketQrPayloads.length),
        total: params.total,
        isReservation: false,
        qrCode: ticketQrPayloads[0],
        attendees: ticketQrPayloads.map((p, i) => ({
          name: `Ticket ${i + 1}`,
          qr_code: p,
        })),
        status: 'Paid',
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      >
          ...
          {!isReservation && loadingIds ? (
            ...
            <View style={styles.qrSection}>
              ...
              <Text style={styles.qrHint}>
                We could not load your ticket codes from the server. Open Bookings and use the QR
                on your ticket there.
              </Text>
            </View>
          ) : qrUriList.length > 0 ? (
            <View style={styles.qrSection}>
        ...
        <View style={styles.actions}>
          ...
          <TouchableOpacity style={styles.actionBtn} onPress={() => void handleDownloadPdf()} activeOpacity={0.8}>
...
const styles = StyleSheet.create({
  ...
  qrHint: { color: COLORS.muted, fontSize: FONT.sm, marginTop: SPACING.sm, textAlign: 'center' },
  qrCodeText: {
